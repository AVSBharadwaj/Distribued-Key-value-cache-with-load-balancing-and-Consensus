...
void *stabilize(void *ptr) {
    while(true) {
       ...
       //     cout << "AFTER READ" << endl;
		...
		//(!) If x.key == local.key, x becomes its own successor, even though there was distinct successor
               // cout << "Successor: " << successor.port << endl;
                // finger_table[1] = x;
            }
...
void *fix_fingers(void *ptr) {
    ...
    while(true) {
        ...
        finger_table[i+1] = fetch_successor(getHashInt(power_2));
        i = (i+1)%16;
        sleep(5);
    }
}
...
// string getHash(char *ip_address, int port) {
//   char port_str[5];
...
Node fetch_successor(string key) {
   // cout << "Inside fetch successoor" << endl;
    if(belongs_to(key, local.key, successor.key)) {
        // return successor
        //cout << "Inside if" << endl;
        return successor;
    }
...
void initialize_chord_ring(int portno) {
    ...
    cout << "Initialize chord: " << local.key << endl;
    strcpy(local.ip_addr, "127.0.0.1");
    for(int i=1; i<=16; i++) {
        finger_table[i].key = "NULL";
    }
